    ...
    def infer_policies(self):
        ...
        for this_policy in range(self.n_policies):
            for future_time in range(1, self.t_horizon):
                ...
                self.G[this_policy] = self.G[this_policy] + np.dot(self.aip_log(self.sparse_O[:, future_time]) - np.transpose(self._mdp.C), self.sparse_O[:, future_time])+ np.dot(np.diagonal(np.transpose(self.likelihood_A)*self.aip_log(self.likelihood_A)),np.reshape(self.post_x[:, future_time, this_policy], (2, 1)))
